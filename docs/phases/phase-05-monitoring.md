...
5. Implement Manager Prometheus API client for summary queries.
...
| Manager Backend | Queries Prometheus API and returns summarized view |
...
|---|---|
| Should Grafana dashboards be generated by Manager or managed externally? | Decide MVP behavior and productization direction |

Expected decision:

- MVP: Manager does not generate Grafana dashboards.
...
7. Manager `/metrics/summary` returns structured response and does not leak secrets.
...
| 0.5 | 2026-06-02 | Added explicit `/metrics -> PodMonitor -> Prometheus -> Manager` closure and clarified that `metrics-server` is supplementary only |
