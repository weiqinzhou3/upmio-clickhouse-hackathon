...
| Grafana dashboards remain externally managed in MVP | Sealed | Phase 05 owns metric exposure, Prometheus discovery/query, and API summary; dashboard lifecycle is a separately versioned observability asset or external platform responsibility |
...
| Should Grafana dashboards be generated by `upm-api-server` or managed externally? | Resolved: MVP does not generate or manage Grafana dashboards. Future dashboards are separately versioned observability assets or externally managed content | Phase 05 decision; Productization |
