...
8. Basic Go Manager Backend APIs.
...
- Healthcheck output, manager responses, and example YAML must not leak Secret content.
...
### 5.8 MVP does not implement full Grafana dashboards or PrometheusRule

Dashboards and alert rules are productization enhancements. MVP only requires metrics to be exposed, scraped, queried, and used for basic summaries.

### 5.9 Do not install ClickHouse directly on host machines

ClickHouse Server, ClickHouse Keeper, Manager, and monitoring components must run inside Kubernetes.
...
When package templates plus manager orchestration are insufficient for complex lifecycle management, consider adding a ClickHouse topology CRD to `compose-operator`.
...
- Manager Backend is stateless or near-stateless in MVP.
...
- Healthcheck/diagnostic reports are generated by Manager. Persistence is minimal in MVP and may be enhanced later.
...
> UPMIO Operator capability foundation + ClickHouse package adaptation + Manager API Server product control plane
...
| Phase 03 | Manager Backend | Implement Go API Server, UPMIO adapter, K8s reader, and ClickHouse client | P0 | Confirmed | [phase-03](phases/phase-03-manager-backend.md) |
...
| Manager Backend uses Go | Sealed | Aligns with Kubernetes, UPMIO, controller-runtime/client-go ecosystem |
| API uses structured error response | Sealed | Required for UI, automation, and troubleshooting |
| Manager uses structured logging with redaction | Sealed | Required for auditability, diagnosis, and secret safety |
| MVP Manager does not require an independent database | Sealed | Keeps MVP stateless and reduces moving parts |
...
| Manager owns validation-only Distributed table initialization in MVP | Sealed | Production-grade behavior must not silently create or mutate user business tables during cluster creation; Manager may create deterministic validation objects for healthcheck |
| Manager should provide controlled database management as a future production capability | Sealed | Enterprise users need API/UI workflows for database lifecycle. User business local table and Distributed table lifecycle remains DBA/application-owned; Manager may create validation objects and read table metadata for diagnostics |
...
| Manager-generated DDL must be idempotent and drift-aware | Sealed | `IF NOT EXISTS` is required but not sufficient; Manager-generated DDL is limited to database lifecycle and validation objects, and must validate existing definitions instead of silently overwriting drift |

## 14. Open Questions

| Question | Current Status | Expected Resolution Phase |
|---|---|---|
| Should Distributed tables be initialized by Manager or left to application users? | Resolved: Manager initializes validation-only Distributed tables for healthcheck during MVP. Production Manager may expose database management API/UI, but user business local table and Distributed table lifecycle remains DBA/application-owned. Manager must not silently create business tables during cluster creation | Phase 02 decision; Phase 04 validation implementation; production database API/UI later |
...
| How should DDL idempotency be guaranteed? | Resolved: use idempotent SQL, record desired database/validation-object target and checksum in future Manager/audit state, and verify Manager-owned objects for drift before applying changes | Phase 02 decision; Phase 04+ implementation |
| Which unit-operator image supports `GrpcCall(type=clickhouse)`? | TBD | Phase 07 |
| Does operation history require persistent storage? | TBD | Phase 03 / Productization |
| Should Grafana dashboards be generated by Manager or managed externally? | TBD | Phase 05 / Future |
