...
| Grafana dashboards are external environment assets in MVP | Sealed | Phase 05 owns metric exposure, Prometheus discovery/query, API summary, and a demo-ready Grafana dashboard provisioned by external kube-prometheus-stack environment scripts; `upm-api-server` does not generate or manage dashboards |
